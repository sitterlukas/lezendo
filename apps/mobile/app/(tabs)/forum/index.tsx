      ...
      <FlatList
        ...
        ListEmptyComponent={
          <EmptyState
            ...
            title="No topics yet"
            message="Start the conversation — ask a question or share some beta."
            actionLabel="Start a topic"
            onAction={() => router.push("/(tabs)/forum/new")}
          />
        }
        renderItem={({ item }) => (
          <Link href={`/(tabs)/forum/${item.id}`} asChild>
            <Pressable
              className={`flex-row items-center gap-3 ${cardPressableClass}`}
            >
