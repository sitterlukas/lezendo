...
import { reviewsQuery } from "@whipperbook/api-client";
...
export default function EntityReviews({
  ...
  const { data, isPending, isError } = useQuery(
    reviewsQuery<EntityReviewDto[]>(browserApi, entityType, entityId),
  );
  ...
  return (
    <section className="mt-12">
      <Header data={data} />
      {isPending ? (
        <p className="mt-5 text-sm text-zinc-500">Loading reviews…</p>
      ) : isError ? (
...
function Header({ data }: { data: EntityReviewDto[] | undefined }) {
  const count = data?.length ?? 0;
  const avg =
    data && count
      ? data.reduce((sum, r) => sum + Number(r.rating), 0) / count
      : 0;
  return (
